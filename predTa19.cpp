    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("predTa19"), path_to_dll, _MAX_PATH))
        return false;
...
{
    return predTa19InitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
    return mclFeval(_mcr_inst, "predictionTa19", nlhs, plhs, nrhs, prhs);
}

LIB_predTa19_CPP_API 
void MW_CALL_CONV predictionTa19(int nargout, mwArray& Y, const mwArray& X)
{
    mclcppMlfFeval(_mcr_inst, "predictionTa19", nargout, 1, 1, &Y, &X);
}
