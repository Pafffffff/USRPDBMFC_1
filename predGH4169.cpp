    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("predGH4169"), path_to_dll, _MAX_PATH))
        return false;
...
{
    return predGH4169InitializeWithHandlers(mclDefaultErrorHandler, 
...
bool MW_CALL_CONV mlxPredictionGH4169(int nlhs, mxArray *plhs[], int nrhs, mxArray 
                                      *prhs[])
{
    return mclFeval(_mcr_inst, "predictionGH4169", nlhs, plhs, nrhs, prhs);
}

LIB_predGH4169_CPP_API 
void MW_CALL_CONV predictionGH4169(int nargout, mwArray& Y, const mwArray& X)
{
    mclcppMlfFeval(_mcr_inst, "predictionGH4169", nargout, 1, 1, &Y, &X);
}
